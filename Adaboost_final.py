...
def preproccess(dataset):
    cl1_data = dataset.loc[dataset.iloc[:, -1] == '+']
    cl2_data = dataset.loc[dataset.iloc[:, -1] == '-']
    # print len(cl1_data)
    # print len(cl2_data)
    dataset = pd.concat([cl1_data, cl2_data], ignore_index=True) #.iloc[:20000]
    dataset[dataset.iloc[:, -1].name] = dataset[dataset.iloc[:, -1].name].replace({'+': 1, '-': -1})
...
dataset = pd.read_csv('./datasets/crx.data.csv')
...
alphas, models = adaboost_train(training_data, training_data.columns[:-1], 5, target_attribute_name=classify_by)
